...
exports.withHttpManagement = lambda_1.curry(__withHttpManagement);
function __withHttpManagement(options, env) {
    ...
    const cache = new Map();
    let cacheClearDisposable = disposable_1.Disposable.None;
    ...
    function scheduleNextClear() {
        ...
        cacheClearDisposable = timer_1.whenIdle(clearAllOldResponses, timer);
    }
    ...
    function http(url, options, callbacks) {
        ...
        return env.http(url, options, {
            ...callbacks,
            success: isCacheable ? lambda_1.pipe(cacheResponseByKey(key), success) : success,
        });
    }
    return {
        http,
